...
class EspoCRMClient:
    """Cliente mínimo y de solo lectura para EspoCRM."""
    ...
    def _get(self, path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if response.status_code >= 400:
            raise EspoCRMError(
                f"EspoCRM respondió HTTP {response.status_code} en {path}: {response.text[:500]}"
            )
        try:
    ...
    def list_leads(self, max_size: int = 200) -> list[dict[str, Any]]:
        ...
        while len(rows) < max_size:
            payload = self._get(
                "Lead",
                {"maxSize": page_size, "offset": offset, "orderBy": "modifiedAt", "order": "desc"},
            )
    ...
    def related(
        ...
        payload = self._get(
            ...
            {"maxSize": max_size, "orderBy": order_by, "order": order},
        )
        return payload.get("list", [])

    def lead_emails(self, lead_id: str, link: str = "emails") -> list[dict[str, Any]]:
        return self.related("Lead", lead_id, link)
